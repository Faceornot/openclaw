...
- A new baseline training run was launched on 2026-03-20 on host `10.168.1.102` for `Isaac-Velocity-Flat-Unitree-Go2-v0` using 4 GPUs and `max_iterations=100`; the run successfully entered training and reached at least iteration 9/100, so the current active direction is to continue with 4-GPU baseline training before any further scale-up.
